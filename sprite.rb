...
class Sprite 
	...
	def initialize(image, x = 0, y = 0, z = 0, angle = 0, zoom_x = 1, zoom_y = 1, opacity = 255, moving = false, in_tool_bar = false)
		@image, @x, @y, @z, @angle, @zoom_x, @zoom_y, @opacity, @moving, @in_tool_bar = image, x, y, z, angle, (image.height/Float(image.height))*zoom_x, (image.width/Float(image.width))*zoom_y, opacity, moving, in_tool_bar
